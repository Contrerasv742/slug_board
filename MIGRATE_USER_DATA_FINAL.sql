-- Final Corrected Migration Script: users table → Profiles table
-- Uses the actual column names from your tables

-- First, let's see both table structures
...
SELECT 
  u.id, 
  u.email, 
  u.full_name, -- users table has full_name
  u.avatar_url, 
...
SELECT 
  id, email, name, provider, last_sign_in, created_at, updated_at
FROM "Profiles" 
LIMIT 5;
