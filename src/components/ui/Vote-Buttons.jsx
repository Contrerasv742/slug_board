...
const UpVotesSection = ({
  ...
  const arrow_img = light
    ? "/images/vote-arrow-black.png"
    ...
    try {
      ...
      if (existingVote) {
        if (existingVote.vote_type === newVoteType) {
          ...
          // Update event count
          const updateField =
            newVoteType === "upvote" ? "upvotes_count" : "downvotes_count";
  ...
  return (
    <div
      className={`flex items-center gap-1 lg:gap-0 p-1 lg:p-0 
        rounded-[15px] lg:rounded-[22px] shadow-sm hover:shadow-md 
        transition-all duration-300 ${getContainerBackground()}`}
    >
      <VoteButton type="up" onClick={handleUpvote} />
      <span
        className={`${getTextColor()} text-xs sm:text-sm ${txt_size} font-normal px-2
                       transition-all duration-300 ${animateCount ? "scale-125 font-bold" : ""}`}
      >
        {netVotes}
      </span>
      <VoteButton type="down" onClick={handleDownvote} />
    </div>
  );
};
