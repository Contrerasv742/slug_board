...
const SearchView = ({ 
  ...
  const baseClasses = 'w-full rounded-[20px] bg-global-2 text-global-2 font-ropa transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500';
  ...
  const textSizeClasses = 'text-base sm:text-lg md:text-2xl lg:text-[40px] lg:leading-[43px]';

  const inputClasses = `${baseClasses} ${paddingClasses} ${textSizeClasses} ${className}`.trim().replace(/\s+/g, ' ');
