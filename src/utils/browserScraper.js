...
class BrowserEventScraper {
  ...
  parseEventbrite(doc) {
    ...
    const eventElements = doc.querySelectorAll(
      '[data-testid="event-card"], .event-card, .search-event-card',
    );
      ...
      try {
        const event = {
          title: this.extractText(
            element,
            'h3, .event-title, [data-testid="event-title"]',
          ),
          ...
          price: this.extractText(
            element,
            '.event-price, [data-testid="event-price"]',
          ),
  ...
  parseMeetup(doc) {
    const events = [];
    const eventElements = doc.querySelectorAll(
      '[data-testid="event-card"], .event-listing, .event-item',
    );
  ...
  parseSantaCruz(doc) {
    ...
    const eventElements = doc.querySelectorAll(
      ".event-item, .calendar-event, .list-item",
    );
    ...
    eventElements.forEach((element) => {
      try {
        const event = {
          ...
          location:
            this.extractText(element, ".location, .venue") || "Santa Cruz, CA",
