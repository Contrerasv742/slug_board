...
# STEP 1: Save events to CSV first for validation
if events:
    df = pd.DataFrame(events)
    df.to_csv('santacruz_events.csv', index=False)
    print(f"Saved {len(events)} events to santacruz_events.csv")
    print("Review the CSV file to validate the scraped data before database insertion.")
else:
    print("No events found. Adjust selectors in the script as needed.")
    exit()

# STEP 2: Insert validated events into Supabase
...
for event in events:
    ...
    try:
        ...
        if "row-level security policy" in str(e):
            ...
            print("  → See RLS_FIX_INSTRUCTIONS.md for detailed solution")
